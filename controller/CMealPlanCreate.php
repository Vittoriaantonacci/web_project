...
class CMealPlanCreate {
    ...
    // Show the creation form with products and recipes to select
    public function showCreateForm(array $error = null): void {
        // Load data for select options (assuming products are images)
        $products = $this->entityManager->getRepository(EImage::class)->findAll();
        $recipes = $this->entityManager->getRepository(ERecipe::class)->findAll();
...
// Example usage:
$controller = new CMealPlanCreate($entityManager, $smarty);
